...
%Joe                                    % Kept a unique com port for our respective
                                        %computers
...
while(get(button, 'UserData'))
    ...
    magdata= sqrt(gxdata.^2+gydata.^2+gzdata.^2);   %calculate the resultant
    ...
    %--->
    plot(index,magdata);        % plotted the resultant
    
    axis([1 buf_len 0 10]);     % tuned the axis
    xlabel('time');
